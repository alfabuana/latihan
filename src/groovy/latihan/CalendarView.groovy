...
class CalendarView extends VerticalLayout{
	def selectedRow
	def itemlist
	...
	public CalendarView() {
		...
		calendar.setHandler(new BasicBackwardHandler(){
					...
					public void backward(BackwardEvent event) {
						...
						initCalendar(cal.getStartDate(),cal.getEndDate())
					}
				})
...
//		calendar.setReadOnly(true)
		initCalendar(calendar.getStartDate(),calendar.getEndDate())
		addComponent(calendar)
		setExpandRatio(menu,1)
		setExpandRatio(calendar,20)
		//
		//		calendar.getHeight().toString()
		//		calendar.setStartDate(new Date(2015,3,1))
		//		calendar.setEndDate(new Date(2015,4,1))
		def FacilityList = Grails.get(FacilityService).getList()
		for (a in FacilityList)
		{
			Styles styles = Page.getCurrent().getStyles()
			styles.add(".v-calendar .v-calendar-event-${a.fieldRGBColor} .v-calendar-event-content { background-color: ${a.fieldCSSColor}}")
		}
	...
	void initCalendar(Date startTime,Date endTime)
	{
		Date start
		Date end
		use(TimeCategory) {
			 start =  startTime - 1.days
			 end = endTime + 1.days
		}
		
		
		def calendarList = calendar.getEvents(start, end)
		for (a in calendarList)
		{
			calendar.removeEvent(a)
		}
		def BookingList = Grails.get(BookingService).getList(startTime,endTime)
		for (Booking a in BookingList)
		{
			long milliseconds = a.startDate.getTime() + (a.startDate.getNanos() / 1000000)
			Date startDate = new Date(milliseconds)
			Date endDate = new Date(milliseconds)
			startDate.setHours(a.startTime.hours)
			endDate.setHours(a.startTime.hours + a.duration)
			BasicEvent event = new BasicEvent();
			event.setCaption(a.customer1.name);
			event.setStart(startDate);
			event.setEnd(endDate);
			event.setStyleName("${a.facility1.fieldRGBColor}");
